...
DIRETORIO_DESTINO <- "LDO-SC"
PAUSA_MIN <- 0.5
PAUSA_MAX <- 2.0

# Lista de tarefas (URLs e seletores)
TAREFAS_SCRAPING <- list(
  list(url = "https://www.alesc.sc.gov.br/orcamento-estadual/ldo?page=46", 
       selector = "#ldo .file a", 
       ano = "2010-2025") # A página 46 parece conter anos mais antigos, mas mantendo a lógica
)
...
#' @title [MODIFICADO] Extrai os links <a> e seus títulos
...
extrair_links_da_pagina <- function(client, url, selector) {
  ...
  dados_extraidos <- lapply(links_elementos, function(el) {
    ...
    # [NOVO] Extrai o atributo 'title'
    titulo <- tryCatch(unlist(el$getElementAttribute("title")), error = function(e) NA_character_) 
...
baixar_arquivo_http <- function(url, pasta_destino, nome_base = NULL) {
  ...
  tryCatch({
    ...
    resp <- httr::GET(url, ua, httr::timeout(60))
    ...
    if (!is.null(nome_base) && nchar(nome_base) > 0) {
      message("   -> 💡 Info: Usando nome do atributo 'title' fornecido.")
...
main <- function() {
  ...
  message("\n✨ Olá! Iniciando a busca pelos arquivos LDO de Santa Catarina (ALESC)... ✨")
  ...
  tryCatch(
    {
      ...
      for (tarefa in TAREFAS_SCRAPING) {
        ...
        # [MODIFICADO] Loop sobre as LINHAS do data.frame
        for (i in 1:nrow(df_links_para_baixar)) {
          ...
          # [MODIFICADO] Passa o 'titulo_arquivo' como 'nome_base'
          baixar_arquivo_http(url = url_arquivo, 
                              pasta_destino = DIRETORIO_DESTINO,
